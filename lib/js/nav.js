// localFlag sets the book to run in either in "local mode" or "online mode"
// if localFlag == false, the book is set to run in online mode where Google Analytics will run
// if localFlag == true, the book is set to run in local mode where Google Analytics will not run
// Google Analytics was causing problems when trying to load from a local source
var localFlag = false;
var ga;

// if we are set to run in local mode, we make a dummy ga() function for all of the labs that call ga()
if (localFlag == true) {
	ga = (function() { /* Do absolutely NOTHING */ });
}
