...
function OrGate4(initX, initY, setName, id, setup) {
	...
	var plugin4 = null;				// the fourth input line
	var plugin4Val = -1;
	...
	var plugin4Comp = null;			// the component that is connected to plugin2
	...
	plugin2 = new Kinetic.Line({
			points : [gScale * 28, gScale * 20, gScale * 61, gScale * 20],
			stroke : 'black',
			strokeWidth : 1,
			lineCap : 'round',
			lineJoin : 'round'
	...
	plugin3 = new Kinetic.Line({
			points : [gScale * 28, gScale * 30, gScale * 61, gScale * 30],
			stroke : 'black',
			strokeWidth : 1,
			lineCap : 'round',
			lineJoin : 'round'
	...
	plugin4 = new Kinetic.Line({
			points : [gScale * 28, gScale * 40, gScale * 58, gScale * 40],
			stroke : 'black',
			strokeWidth : 1,
			lineCap : 'round',
			lineJoin : 'round'
	...
	function draw() {
		...
		group.add(plugin4);		// ... the fourth plugin line
	...
	function getConnectorPlugin(num) {
		...
		else if (num == 4) return connectorPlugin4;
	}
	...
	function getPluginComp(num) {
		...
		else if (num == 4) return plugin4Comp;
	}
	...
	function setPlugoutWire(line) {
		plugoutWire = line;
		if (plugin1Val == 1 || plugin2Val == 1 || plugin3Val == 1 || plugin4Val == 1) plugoutWire.setStroke("red");
	...
	function setPluginComp(num, comp) {
		...
		else if (num == 4) plugin4Comp = comp;
	...
	function setPluginVal(comp, val) {
		if (comp == plugin1Comp && comp == plugin2Comp && comp == plugin3Comp && comp == plugin4Comp) { 
			...
			if (val == -1 || val == 0) { plugin1.setStroke("blue"); plugin2.setStroke("blue"); plugin3.setStroke("blue"); plugin4.setStroke("blue"); }
			else { plugin1.setStroke("red"); plugin2.setStroke("red"); plugin3.setStroke("red"); plugin4.setStroke("red"); }
